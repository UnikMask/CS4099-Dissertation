...
\begin{algorithm}
\caption{Rabin Scott's Superset Construction Algorithm}\label{rabinscott}
\begin{algorithmic}[1]
...
algorithms than the rest. Those are Hopcroft's algorithm and Brozozowski's minimization algorithm.
...
\begin{algorithm}
\caption{Hopcroft's Algorithm}\label{hopcroft}
\begin{algorithmic}[1]
\Procedure{HopcroftAlgo}{\(M = (S, \Sigma, \delta, s_{0}, T)\)}
    ...
    \While{\( \left| Q \right| \neq 0\)}
        ...
        \ForAll{a \in \(\Sigma\), V \in \(P\)}
            \If{\(\delta^{-1}(P_{next}) \cap V \neq \emptyset \cap V \backslash \delta^{-1}(P_{next}) \neq \emptyset\)}
                \State remove \(V\) from \(P\)
                \State push \(\delta^{-1}(P_{next}) \cap V\) into \(P\)
                \State push \(V \backslash \delta^{-1}(P_{next})\) into \(P\)
                \If{\(V \in Q\)}
                    \State replace \([V]\) in \(Q\) with \([V \backslash \delta^{-1}(P_{next}), \delta^{-1}(P_{next}) \cap V]\)
                \ElsIf{\(\left| V \backslash \delta^{-1}(P_{next}) \right| \leq \left| \delta^{-1}(P_{next}) \cap V \right|\)}
                    \State add \(V \backslash \delta^{-1}(P_{next})\) to \(Q\)
                \Else
                    \State add \(\delta^{-1}(P_{next}) \cap V\) to \(Q\)
                \EndIf
            \EndIf
        \EndFor
    \EndWhile
