...
DFA determinization's /Rabin-Scott superset construction/ algorithm is a well-known determinization algorithm which has existed for a long time. However, DFA
...
larger number of \(\epsilon\) transitions.
#+begin_src python :async yes :results value :exports none
import subprocess
import json

arr = [["b", "s", "two-stack determinize", "bns determinize", "two-stack minimize", "bns minimize"]]
fp_orig = "/cs/home/apjg1/Documents/gitrepos/nfdeterminize/target/criterion"
estim_loc = "base/estimates.json"
for i in range(2, 4):
    for j in range(2, 6):
        ...
        p = subprocess.run(["./nfdeterminize", "--verbose", "--timed", "--file", "/dev/null", "run", "file", fp], capture_output=True)
        stdout = p.stdout.decode().split('\n')
        ngaps = int(stdout[0].split(' ')[-1])
        t_gaps = 0.0
        with open(f"{fp_orig}/determinize file bns {i} {j} Sequential/{estim_loc}") as f:
           estimates = json.load(f)
           t_gaps = float (estimates["mean"]["point_estimate"]) / 1000
        arr.append([i, j, nselfs, ngaps, round(t_selfs, 2), round(t_gaps, 2)])
return arr
#+end_src
